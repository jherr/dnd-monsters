...
export const fields = Object.keys(items[0]).filter((k) => k !== 'name');

export const findRanges = (selection) => {
  const out = {};
  fields.forEach((k) => {
    out[k] = { min: 10000, max: 0 };
  });
...
export const startItem = (itemList.filter(({ name }) => name === startingItemName) || itemList)[0];

const tree = createKDTree(itemList.map((item) => fields.map((f) => item[f])));
